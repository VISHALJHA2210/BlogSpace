...
const serviceAccountKey = JSON.parse(fs.readFileSync('./mern-blogging-d64fc-firebase-adminsdk-d1sa3-26404c1735.json', 'utf8'));
...
server.post("/signup",(req,res)=>{
    ...
    if(fullname.length < 3){
        return res.status(403).json({"error":"Fullname must be at leat 3 letters long"});
    }
    if(!email.length){
...
server.post("/change-password", verifyJWT ,(req,res)=>{
    ...
    .then((user)=>{
        if(user.google_auth){
            return res.status(403).json({error:"you can't change Accoount's password because you logged in through google"})
        }
...
server.post("/update-profile-img", verifyJWT, (req,res)=>{
    ...
    let {imgUrl} = req.body;
    User.findOneAndUpdate({_id:req.user},  { "personal_info.profile_img":imgUrl })
    .then(()=>{
        return res.status(200).json({profile_img:imgUrl})
    })
    .catch(err=>{
        return res.status(500).json({error:err.message})
    })
...
server.post("/update-profile", verifyJWT, (req,res)=>{
    ...
    if(bio.length > bioLimit){
        return res.status(403).json({error:"Bio should not exceed 150 characters"})
    }
        ...
        for(let i=0;i<socialLinksArr.length;i++){
            if(social_links[socialLinksArr[i]].length){
                ...
                if(!hostname.includes(`${socialLinksArr[i]}.com`) && socialLinksArr[i]!='website'){
                    return res.status(403).json({error:`${socialLinksArr[i]} link is invalid. You must enter a correct link`})
                }
            }   
        }
...
server.post("/add-comment", verifyJWT, (req, res)=>{
    ...
    new Comment(commentObj).save().then(async commentFile=>{
        ...
        new Notification(notificationObj).save().then(notification=>console.log('new notif created'));
...
const deleteComments = (_id)=>{
    ...
    .then(comment=>{
        ...
        Blog.findOneAndUpdate({_id:comment.blog_author}, {$pull : {comments:_id}, $inc:{"activity.total_comments":-1, "activity.total_parent_comments": comment.parent ? 0 : -1}})
        .then(blog=>{
...
server.get("/new-notification", verifyJWT, (req, res)=>{
    ...
    .then(result=>{
        if(result){
            return res.status(200).json({new_notif_available:true})
        }else{
            return res.status(200).json({new_notif_available:false})
        }
    })
    .catch(err=>{
        console.log(err)
